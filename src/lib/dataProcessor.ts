...
export function processLabTestExcel(buffer: Buffer) {
  ...
  return {
    ...
    thresholds: {
      ...
      rc: final_rc,
      max: final_max,
      calculatedAt: (calculated_at_min !== null && Math.abs(calculated_at_min - final_at) > 0.01) ? calculated_at_min : null,
      calculatedRc: (calculated_rc_min !== null && Math.abs(calculated_rc_min - final_rc) > 0.01) ? calculated_rc_min : null,
      calculatedMax: (calculated_peak_vo2_min !== null && Math.abs(calculated_peak_vo2_min - final_max) > 0.01) ? calculated_peak_vo2_min : null,
      fatMax: fatmax_min,
      fatMaxHr: fatmax_hr,
      fatMaxZoneStartHr,
      fatMaxZoneEndHr,
