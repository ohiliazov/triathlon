...
export default function LabTestPage() {
  const { labTestData, setLabTestData } = useAppContext();
  const router = useRouter();

  const data = labTestData?.data || null;
  const steadyData = labTestData?.steadyStateData || null;
  const thresholds = labTestData?.thresholds || null;
  ...
  useEffect(() => {
    if (data) {
      const hasSpeed = data.some((d: any) => (d.Speed ?? 0) > 0);
      const hasPower = data.some((d: any) => (d.Power ?? 0) > 0);
